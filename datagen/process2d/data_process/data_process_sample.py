"""
Sample the final object points and write the training-ready track file.
"""
...
def process_unique_points(track_data):
    ...
    video_writer = imageio.get_writer(
        f"{base_path}/{case_name}/sampled_pcd.mp4", fps=30
    )
...
def visualize_track(track_data):
    ...
    vis.create_window(visible=False)
    video_writer = imageio.get_writer(
        f"{base_path}/{case_name}/sampled_tracks.mp4", fps=30
    )
...
if __name__ == "__main__":
    with open(f"{base_path}/{case_name}/filtered_tracks.pkl", "rb") as f:
        track_data = pickle.load(f)

    track_data = process_unique_points(track_data)

    with open(f"{base_path}/{case_name}/sampled_tracks.pkl", "wb") as f:
