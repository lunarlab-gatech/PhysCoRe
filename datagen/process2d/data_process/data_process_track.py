"""
Filter the tracked points and turn them into consistent 3D trajectories.
"""
...
parser.add_argument("--controller", type=str, default="hand",
                    help="Manipulator name. A human 'hand' keeps the strict 30-point "
                         "controller requirement; any other controller (e.g. a robot "
                         "gripper) uses min(30, available) so sparser/occluded tracks pass.")
...
CONTROLLER_IS_HAND = args.controller.strip().lower() == "hand"
...
def filter_track(track_path, pcd_path, mask_path, frame_num, num_cam):
    ...
    for i in range(num_cam):
        ...
        H, W = processed_masks[0][i]["object"].shape[:2]
...
def filter_motion(track_data, neighbor_dist=0.01):
    ...
    for i in tqdm(range(num_frames - 1)):
        ...
        # Get the neighbors for each points and filter motion based on the motion difference between neighbors and the point
        for j in range(num_points):
...
def get_final_track_data(track_data, controller_threhsold=0.01, controller_is_hand=True):
    object_points = track_data["object_points"]
    object_colors = track_data["object_colors"]
    object_visibilities = track_data["object_visibilities"]
    ...
    available = len(new_controller_points[0])
    if controller_is_hand:
        assert available >= 30
        num_controller = 30
    else:
        # Non-hand controllers (e.g. a robot gripper) track sparsely and get
        # occluded on long sequences; take what's available up to 30.
        assert available >= 1, "no valid controller points after filtering"
        num_controller = min(30, available)
    ...
    fps_pcd = sample_pcd.farthest_point_down_sample(num_controller)
    final_indices = []
...
if __name__ == "__main__":
    ...
    track_data = get_final_track_data(track_data, controller_is_hand=CONTROLLER_IS_HAND)

    with open(f"{base_path}/{case_name}/filtered_tracks.pkl", "wb") as f:
