"""
Differentiable SVD implemented with a Warp kernel.
"""
...
class SVDFunction(autograd.Function):
    ...
    def forward(ctx: autograd.function.FunctionCtx, F: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        F_contiguous = F.contiguous()
        A_grad = torch.zeros_like(F_contiguous) if requires_grad else None
    ...
    def backward(ctx: autograd.function.FunctionCtx, grad_U: Tensor, grad_sigma: Tensor, grad_Vh: Tensor) -> tuple[Tensor]:
        ...
        U.grad = wp.zeros_like(U) if grad_U is None else wp.from_torch(
            grad_U.contiguous(),
            dtype=wp.mat33,
            requires_grad=False,
        )
        sigma.grad = wp.zeros_like(sigma) if grad_sigma is None else wp.from_torch(
            grad_sigma.contiguous(),
            dtype=wp.vec3,
            requires_grad=False,
        )
        Vh.grad = wp.zeros_like(Vh) if grad_Vh is None else wp.from_torch(
            grad_Vh.contiguous(),
            dtype=wp.mat33,
            requires_grad=False,
        )
